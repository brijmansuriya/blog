                    ...
                    @foreach($category->subcategory as  $subcategorys)
                        <ul class="sidebar-submenu collapse sm-indent" id="enterprise_menu">
                            <li class="sidebar-menu-item">
                                ...
                                @if(count($subcategorys->game)>0)
                                 href="{{route('subcategoryview',['category'=> $subcategorys->cid,'subcategory'=>$subcategorys->id])}}"
                                @endif
                                 >
                            ...
                            @foreach($subcategorys->game as $game)
                                <ul class="sidebar-submenu collapse sm-indent" id="enterprise_menu">
                                    <li class="sidebar-menu-item">
                                        <a class="sidebar-menu-button" href="{{route('subcategoryview',['category'=> $game->cid,'subcategory'=>$game->scid])}}">{{$game->name}}</a>
                                    </li>
                                </ul>
                            @endforeach
                            </li>
